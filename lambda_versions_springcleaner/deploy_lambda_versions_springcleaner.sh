...
LAMBDA_FUNCTION_NAME="lambda-versions-springcleaner"
LAMBDA_EXECUTION_ROLE="lambda-versions-springcleaner-role"
...
POLICY_ARN=$(aws iam create-policy \
    --policy-name "LVSAPIAccessPolicy" \
    --policy-document '{
    	"Version": "2012-10-17",
    	"Statement": [
        	{
	...
	--query "Policy.Arn" \
	--output text) || handle_error "Failed to create LVSAPIAccessPolicy policy."

echo "LVSAPIAccessPolicy policy created successfully."

# Attach the LVSAPIAccessPolicy policy to the Lambda Execution Role.
aws iam attach-role-policy \
    --policy-arn "$POLICY_ARN" \
    --role-name "$LAMBDA_EXECUTION_ROLE" || handle_error "Failed to attach LVSAPIAccessPolicy to the Lambda Execution Role."
